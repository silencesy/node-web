				...
				>
					...
					>
						<view class="i-top b-b">
							<text class="time">订单号: {{item._id}}</text>
						...
						>
							...
							<view class="right">
								...
								<text class="attr-box">{{goodsItem.difference.join(' ')}}  x {{goodsItem.number}}</text>
						...
						<view class="price-box">
							...
							件商品 实付款
							<text class="price">{{item.total_order_price}}</text>
						</view>
						<view class="action-box b-t" v-if="item.status === 1">
							<button class="action-btn" @click="cancelOrder(item)">取消订单</button>
							<button class="action-btn recom">立即支付</button>
							<button class="action-btn recom" @click="goOrderDetials(item)">订单详情</button>
						</view>
						<view class="action-box b-t" v-if="item.status !== 1">
							<button class="action-btn recom" @click="goOrderDetials(item)">订单详情</button>
						</view>
	...
	export default {
		...
		methods: {
			...
			async loadData(source){
				...
				let { data: { docs, total} } = await this.Api.orderList(params);
