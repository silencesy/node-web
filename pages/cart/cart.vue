				...
				<block v-for="(item, index) in cartList" :key="item._id">
					...
					>
						...
						<view class="item-right">
							...
							<text class="attr">{{item.difference.join(' ')}}</text>
			...
			<view class="action-section">
				<view class="checkbox">
					<image 
						...
						@click="check('all')"
					></image>
					<view v-show="allChecked" class="clear-btn" :class="{show: allChecked}" @click="clearCart">
						清空
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			async loadData(){
				...
				let { data,code } = await this.Api.getCartList();
				if(code == 1) {
					this.cartList = data;
					this.calcTotal();  //计算总价
				}
			...
			async check(type, index){
				...
				if(type === 'item'){
					...
					await this.Api.editCart(params);
			...
			async numberChange(data){
				let params = {
					number: data.number,
					_id: this.cartList[data.index]._id
				}
				let data2 = await this.Api.editCart(params);
